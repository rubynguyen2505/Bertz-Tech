...
namespace AKCS
{
    public class SlotManager : MonoBehaviour
    {
        ...
        private void Awake() {
            ...
            _slot = new Slot[slotButton.Length];

            for (int i = 0; i < slotButton.Length; i++)
            {
                _slot[i] = slotButton[i].GetComponent<Slot>();
            }
        ...
        private void OnEnable() 
        {
            ...
            //
            for(int i=0;i<characterManager.cards.Length;i++)
            {
                characterManager.cards[i].inTeam = false;
            }
            ...
            if(teamManager.listTeam.Count > 0)
            {
                ...
                foreach(Cards c in teamManager.listTeam)
                {
                    ...
                    //
                    Cards card = Array.Find(characterManager.cards, _card => _card == c); 
                    card.inTeam = true;
                }
            }
        }
